...
export const SettingsPanel = ({
  ...
  const [alternateBlockchainUrl, setAlternateBlockchainUrl] = useState('');
  ...
  const [useAlternateBlockchain, setUseAlternateBlockchain] = useState(false);
  ...
  const getFullBlockchainUrl = () => {
    if (useAlternateBlockchain && alternateBlockchainUrl) {
      return alternateBlockchainUrl;
    }
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-1 text-xs">
                          <Button
                            ...
                            className="text-xs h-7 px-2"
                            onClick={() => blockchainInputMode === 'host' ? setBlockchainInputMode('full') : null}
                          >
                            <ExternalLink className="h-3 w-3 mr-1" />
                            Full URL
                          </Button>
                          <Button
                            ...
                            className="text-xs h-7 px-2"
                            onClick={() => blockchainInputMode === 'full' ? setBlockchainInputMode('host') : null}
                          >
                            <Server className="h-3 w-3 mr-1" />
                            Host + Path
                          </Button>
                    ...
                    <div className="space-y-2 mt-4 border-t border-border pt-4">
                      <div className="flex items-center justify-between">
                        <Label htmlFor="alt-blockchain" className="flex items-center gap-2 text-xs">
                          <Link2 className="h-3.5 w-3.5" />
                          Use Alternate Blockchain Source
                        </Label>
                        <Switch 
                          id="alt-blockchain" 
                          checked={useAlternateBlockchain} 
                          onCheckedChange={setUseAlternateBlockchain} 
                        />
                      </div>
                      
                      {useAlternateBlockchain && (
                        <div className="space-y-1 mt-2">
                          <Label htmlFor="alt-blockchain-url" className="text-xs text-muted-foreground">
                            Alternate Blockchain URL
                          </Label>
                          <Input
                            id="alt-blockchain-url"
                            value={alternateBlockchainUrl}
                            onChange={(e) => setAlternateBlockchainUrl(e.target.value)}
                            placeholder="https://alternate-blockchain-api.com/chain"
                            className="font-mono text-sm"
                          />
                          <p className="text-xs text-muted-foreground mt-1">
                            This URL will be used instead of the primary blockchain URL if enabled
                          </p>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="pt-2">
                    <Button
                      ...
                      className="w-full"
                      disabled={
                        (apiInputMode === 'full' ? !apiUrl : !apiHost) || 
                        (useAlternateBlockchain ? !alternateBlockchainUrl : 
